...
- 规则引擎为主，LLM 可选增强
- 面向「可检测、可复现、可自动化」
- 规则引擎必须能在无 LLM 时独立运行且结果一致
- LLM 只能附加解释，不能产生或屏蔽 finding
- `--llm` 标志需显式传入
- 做检测，不做自动修复
...
```
review 输出 = 规则引擎(files) + [--llm ? LLM(规则引擎输出) : ∅]
```

| 层面 | 是否接入 | 理由 |
|------|----------|------|
| **检测层 (detector)** | ❌ 永不 | finding 产生必须完全确定、可复现 |
| **解释层 (output)** | ✅ `--llm` 标志 | 在已有 finding 上附加解释，不影响 finding 集合 |
| **决策层 (config/rules)** | ❌ 永不 | 启用/禁用规则、严重级别、排除配置必须确定性 |
