...
以下由实验室成果迁移的 D1–D7 决策推测得出（决策记录见 [DECISIONS](./DECISIONS.md)，工作清单见 [TODO](./TODO.md)）。项目处于早期，更新可以激进，不为旧契约、死代码或临时产物留包袌：

- 以原始意图为准——选项与建议不符意图时改选，D1 取「先公开 `pub mod reflect` 再写薄 example」，放弃内联自包含；
- 消除临时产物与死代码——D2 直接删除实验室 `cross_function_slice`，D5 不留输出快照，example 不做自包含副本；
- 能力做完整而非留待办——D6 选补齐 `trace_variable` 跨函数追踪，不接受只标注限制；
- 不迁就旧契约——D4 选重新设计 `graph` 的 JSON，不采用追加字段的兼容做法；
- 未定则先搁置——D3 把 `compute_confidence` 先放进 example，待语义统一后再定归属层；
- 遵循既有规则——D7 按仓库「提交即推送」执行，并同步父仓库指针；
- 先文档后代码，逐项拍板，决策留痕。
