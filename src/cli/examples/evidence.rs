...
//!
//! 1. **证据计数**（确定性，零 LLM）——经 lib `evidence::{count_evidence, anchor_level}`
//!    统计结论文本中的行号引用（L3 / 行 / line）与领域变量名，按合计分
//!    `anchored` / `partial` / `unanchored`；
//! 2. **证据链实验**（需要 LLM）——对样例取反向切片，装入 `CodeEvidenceChain`
//!    得到同一组证据的两种排序（正向 = 执行顺序、反向 = 追溯顺序），分别喂给
//!    LLM 对比两边发现，并用 lib 评证给两份输出打锚定分级。
//!
//! 计数不是 confidence：既不检查证据是否支持主张，也没有校准；与生产
//! `src/llm.rs` 的 `confidence` 字段（LLM 自评 confirm/dismiss）是两回事。
//! 评证与链的实现、计数器的迁入出处见 lib `src/evidence.rs` 模块注释。
//!
...
use qtcloud_code_cli::evidence::{CodeEvidenceChain, anchor_level, count_evidence};
...
fn main() {
    ...
    // 同一组证据装入链：正向 = 行号升序（执行顺序），反向 = 追溯顺序
    let chain = CodeEvidenceChain {
        source: "sample.rs:L6".to_string(),
        target: "process_order 的数据流".to_string(),
        items: slice.iter().cloned().map(Into::into).collect(),
    };
    let rev_chain = chain.reversed();
