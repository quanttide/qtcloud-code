...
步骤说明见 [ROADMAP](./ROADMAP.md)，待拍板决策见 [DECISIONS](./DECISIONS.md)。
...
- [ ] 合并三处重复的 `walk_all`
- [ ] `src/lib.rs` 增加 `pub mod reflect;`（D1）
- [ ] 迁实验室已有 `#[cfg(test)]` 单元测试
- [ ] 删除实验室 `cross_function_slice` 及其专用辅助函数（D2）
...
- [ ] `examples/confidence.rs`：内联演示 `compute_confidence`（D3）
- [ ] `cargo build --examples` 通过

## 阶段二 重构 main.rs

- [ ] 以 git 历史中的既有实现与 `tests/reflect.rs` 为对照基准，不额外留存快照（D5）
- [ ] `run_reflect_slice` 改调 `reflect::backward_slice`，参数、文本输出与退出码保持
- [ ] `run_reflect_trace` 改调 `reflect::trace_variable`，补齐跨函数追踪（D6）
- [ ] `run_reflect_graph` 改调 `reflect::build_call_graph`，JSON 重新设计（D4）
...
- [ ] 依据 example 表现决定 `compute_confidence` 归属层（D3 后续）
- [ ] README 补充新增 reflect 能力
- [ ] AGENTS.md 更新模块结构
- [ ] CHANGELOG 增加条目
- [ ] ROADMAP 登记本轮未完成项
- [ ] `apps/qtcloud-code` 子模块提交并推送，再更新父仓库指针（D7）
