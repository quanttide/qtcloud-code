...
给定 review 的证据（finding），不是停留在"这里有问题"，而是回答"为什么会这样"。
...
用于 reflect：给定一个 unsafe 块、空指针、或任何 finding，反向切片找到所有导致它的路径。
...
```
LLM 输入：evidence_chain（程序切片 + 数据流 + 依赖图）
LLM 输出：自然语言根因解释

"api/handler.rs 和 service/processor.rs 都绕过 data/buffer.rs
直接调用了 data/pointer.rs 的裸指针操作。
因为 buffer 层的 safe 封装不支持批量操作，
两个模块各自手写了 unsafe 实现。

根因：buffer 层的 safe API 覆盖不足。
行动：扩展 buffer 层批量操作接口。
"
```

LLM 的解释是**附着在证据链上**的，没有证据链时 LLM 不生成解释。
  ...
  "investigations": [
    {
      ...
      "evidence_chain": {
        ...
        "data_flow": {"path": ["parse_input", "to_raw_ptr", "buffer_write"], "length": 5},
...
```sh
review . --reflect               # review + reflect（机械侦探）
review . --reflect --llm         # review + reflect + LLM 因果解释
```

无 LLM 时 reflect 仍可独立运行——输出证据链，只是没有自然语言解释。
