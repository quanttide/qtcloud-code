...
规则引擎的命中是**问题层的 finding 候选**——criterion 即规则，命中的代码行原文与度量本该作为 `evidence[]` 挂在 finding 上（现挤在 `message` 里，待信封适配补齐）；LLM 的 priority/explanation/confirm·dismiss 与语义 finding 属**解释层**——在证据之上判断，不反向充当证据。finding 的 `file` + `line` 是 reflect 取证入口——为它补 `evidence[]`，补齐才算合格发现（接线登记下轮，见 [index.md](index.md) 证据主线）。audit 的问题清单与 findings 并列出问题——对齐轴出问题清单、质量轴出 findings，同属问题层、共用 `file` + `line` 取证入口。
