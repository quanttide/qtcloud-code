...
fn all_rule_ids() -> Vec<&'static str> {
    qtcloud_code_cli::review::all_rule_ids()
}
    ...
    let result = match cli.command {
        ...
        Commands::Reflect { action } => match action {
            ...
            ReflectAction::Trace {
                ...
                var,
                json,
            } => run_reflect_trace(file, line, var, json),
...
fn run_review(
    ...
    let all_findings = qtcloud_code_cli::review::collect_findings(&root, &cli_rules)?;
...
fn write_status_file(root: &Path, findings: &[CodeFinding]) -> Result<(), String> {
    ...
    let file =
        std::fs::File::create(&status_path).map_err(|e| format!("无法创建 STATUS.md: {}", e))?;
...
fn find_project_root(path: &Path) -> Option<PathBuf> {
    qtcloud_code_cli::review::find_project_root(path)
}
...
fn run_list_rules(json: bool) -> Result<bool, String> {
    ...
    if json {
        ...
        let detector_rules: Vec<&str> = all_rules
            .iter()
            .filter(|r| **r != "unused-variable" && **r != "missing-tests")
            .copied()
            .collect();
        let compiler_rules = vec![
            (
                qtcloud_code_cli::detector::unused_variable::RULE_ID,
                qtcloud_code_cli::detector::unused_variable::DESCRIPTION,
            ),
            (
                qtcloud_code_cli::detector::missing_tests::RULE_ID,
                qtcloud_code_cli::detector::missing_tests::DESCRIPTION,
            ),
        ];
        print!(
            "{}",
            qtcloud_code_cli::contract::list_terminal(&detector_rules, &compiler_rules)
        );
    }
    Ok(true)
}
...
fn run_refactor_rename(
    ...
    let source =
        std::fs::read_to_string(path).map_err(|e| format!("无法读取文件 {}: {}", file, e))?;
    ...
    let replacements =
        qtcloud_code_cli::refactor::rename::rename_symbol(&table, &old_name, &new_name);
...
fn run_scaffold_tests(
    ...
    let lang = match lang {
        ...
        None => match qtcloud_code_cli::scaffold::detect_lang_from_doc(&combined) {
            ...
            None => {
                return Err(
                    "无法从文档检测语言（无代码块标注），请用 --lang 指定（rs/py/go/ts）"
                        .to_string(),
                );
            }
        },
...
fn run_scaffold_code(
    ...
    let lang = match lang {
        ...
        None => match ext {
            ...
            _ => {
                return Err(format!(
                    "无法从扩展名 .{} 推断语言，请用 --lang 指定（rs/py/go/ts）",
                    ext
                ));
            }
        }
...
fn run_reflect_suggest(file: String, json: bool) -> Result<bool, String> {
    let source = std::fs::read_to_string(&file).map_err(|e| format!("读取文件失败: {}", e))?;
    let suggestions = qtcloud_code_cli::reflect::suggest(&source);
    ...
    for s in &suggestions {
        println!("  L{} [{}] {}", s.line, s.kind, s.text);
    }
