...
fn test_scaffold_code_from_python_tests() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("def add(a0, a1) -> None:"),
        "got: {}",
        stdout
    );
    assert!(stdout.contains("raise NotImplementedError(\"add 待实现\")"));
    assert!(
        stdout.contains("def div(a0, a1) -> None:"),
        "got: {}",
        stdout
    );
    assert!(
        stdout.contains("测试驱动"),
        "应标注生成来源, got: {}",
        stdout
    );
}
...
fn test_scaffold_code_from_rust_tests_macro_calls() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("pub fn add(a0: i32, a1: i32) -> i32"),
        "got: {}",
        stdout
    );
...
fn test_scaffold_code_go_lang_flag() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("func Add(a0 int, a1 int) int"),
        "got: {}",
        stdout
    );
}
...
fn test_scaffold_code_only_external_calls_exit_one() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("未找到测试引用的项目 API"),
        "got: {}",
        stderr
    );
}
